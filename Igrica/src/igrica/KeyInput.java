...
public class KeyInput extends KeyAdapter {
	...
	for (int i = 0; i < handler.object.size(); i++) {
			...
			if (tempObject.getId() == ID.Player) {  //player1 strelice za igru
	...
	if (key == KeyEvent.VK_ESCAPE) System.exit(1);                          //udari Space da uðeš/izaðeš u/iz prodavnice
	...
	for (int i = 0; i < handler.object.size(); i++) {
			...
			if (tempObject.getId() == ID.Player) {  //player1
				...
				if (!keyDown [0] && !keyDown [1]) tempObject.setVelY(0); // vertikalni pokret
				if (!keyDown [2] && !keyDown [3]) tempObject.setVelX(0); // horizontalni pokret
			}
