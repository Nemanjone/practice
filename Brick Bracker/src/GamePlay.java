...
public class GamePlay extends JPanel implements KeyListener, ActionListener {
	...
	private int delay = 4;                                       // a dok se ja setim šta je ...
    ...
    public void paint(Graphics g) {
    	...
    	if(totalBricks <= 0) {                         //ako si uništio sve cigle, pobeda!!!
    	...
    	if (balposY > 570) {                            //ako loptica proðe dole - kraj igre
    		play = false;
    		ballXdir = 0;
    		ballYdir = 0;
    		g.setColor(Color.RED);
    		g.setFont(new Font("serif",Font.BOLD, 30));
    		g.drawString("Igra je završena!",220, 300);
    		g.drawString("Rezultat: " + score, 245, 335);
				...
				for (int j = 0; j < map.map[0].length; j++) {
					if (map.map[i][j] > 0) {
						...
						if (ballRect.intersects(brickRect)) {                                    // kad loptica udari u ciglu
							map.setBrickValue(0, i, j);											 // uništi je!!!	
							...
							break A;                                             // izaði iz spoljne petlje
						}
	...
	public void keyPressed(KeyEvent dogaðaj) {
		...
		if(dogaðaj.getKeyCode() == KeyEvent.VK_RIGHT) {  //da ne preðe granicu okvira
			if(playerX >= 600) {
				playerX = 600;
			}
			else {
	...
	if(dogaðaj.getKeyCode() == KeyEvent.VK_LEFT) {
		if(playerX <10) {
			playerX = 10;
		}
		else {
	...
	if (dogaðaj.getKeyCode() == KeyEvent.VK_ENTER) {  // ako pritisnes enter da igra opet poène
		if (!play) {
			play = true;
			ballposX = 120;
			balposY = 320;
